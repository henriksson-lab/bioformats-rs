...
impl FormatReader for Ecat7Reader {
    ...
    fn set_id(&mut self, path: &Path) -> Result<()> {
        ...
        let mut image_count = num_planes * num_frames;
        let mut data_offset = 1024u64;

        // ECAT7 volume image files commonly store a 512-byte image subheader
        // between the directory block and the matrix data. Prefer dimensions
        // inferred from the matrix payload over the placeholder 128x128 path.
        for (candidate_offset, candidate_width, candidate_height) in [
            (1536u64, 512u32, 512u32),
            (1536, 256, 256),
            (1024, 128, 128),
        ] {
            if file_len <= candidate_offset {
                continue;
            }
            let plane_bytes = candidate_width as u64 * candidate_height as u64 * bps;
            let payload_bytes = file_len - candidate_offset;
            if plane_bytes > 0 && payload_bytes % plane_bytes == 0 {
                let planes = payload_bytes / plane_bytes;
                if planes > 0 && planes <= u32::MAX as u64 {
                    width = candidate_width;
                    height = candidate_height;
                    image_count = planes as u32;
...
fn parse_fdf_header(path: &Path) -> Result<(u32, u32, u32, PixelType, u8, u64)> {
    ...
    for line in text.lines() {
        ...
        if lo.contains("ro_size") {
            ...
            if let Some(&v) = parts.last() {
                if let Ok(n) = v.parse::<u32>().or_else(|_| Ok::<u32, ()>(1)) {
                    slices = n.max(1);
                }
            }
